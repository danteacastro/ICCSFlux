...
Adapted from cRIO node MQTT interface. GC nodes are read-only
(config, commands, and discovery only - no session/script/safety topics).
"""
...
class MQTTInterface:
    ...
    def __init__(self, config: MQTTConfig):
        ...
        # Topics to subscribe on connect (set for dedup)
        self._subscriptions: List = []
        self._subscription_set: set = set()

    @property
    def topic_base(self) -> str:
        """Get node-prefixed topic base (e.g., 'nisystem/nodes/gc-001')."""
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # Set last will (offline status) — node_type 'gc' for GC nodes
    ...
    def subscribe(self, topic_pattern: str, qos: int = 1):
        ...
        """
        key = (topic_pattern, qos)
        if key in self._subscription_set:
            return  # Already subscribed
        self._subscriptions.append(key)
        self._subscription_set.add(key)
    ...
    def setup_standard_subscriptions(self):
        """
        Set up standard subscriptions for GC node.

        GC nodes are read-only analytical instruments. They only need:
        - config/#   : Configuration updates from DAQ service
        - commands/# : Output commands (e.g., trigger analysis run)
        - discovery/ping : Network discovery

        Excluded (not applicable to GC nodes):
        - session/#  : Session lifecycle (managed by DAQ service)
        - script/#   : Script execution (no user scripts on GC)
        - safety/#   : Safety interlocks (GC has no safety outputs)
        - alarm/#    : Alarm acknowledgements (GC only reports data)
        - system/#   : System commands like acquire start/stop
        """
        base = self.topic_base

        # Config updates from DAQ service
        self.subscribe(f"{base}/config/#")

        # Output commands (e.g., trigger analysis, method selection)
