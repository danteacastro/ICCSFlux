"""
Opto22 Node - Script Execution Engine
...
logger = logging.getLogger('Opto22Node.Scripts')
...
# These classes are only available on the DAQ service, not on Opto22.
# =============================================================================

def _daq_only_class(name):
    """Create a stub class that raises RuntimeError when instantiated on Opto22."""
    def _init(self, *args, **kwargs):
        raise RuntimeError(
            f"{name} is only available on the DAQ service (not Opto22). "
...
class ScriptEngine:
    """
    Manages Python script execution on the Opto22.
    ...
    def __init__(self, node):
        ...
        Args:
            node: Opto22Node instance (provides channel values, MQTT, state, etc.)
        """
        self._node = node
