...
import HeaderUserControls from './components/HeaderUserControls.vue'
...
// Initialize the theme early (side effect applies the saved theme before paint).
// The toggle + user controls now live in HeaderUserControls.
useTheme()
  ...
  <div class="app">
    ...
    <header v-if="!store.pidEditMode" class="app-header">
      ...
      <div class="header-right">
        ...
        <HeaderUserControls @login="showLoginDialog = true" />
      </div>
    </header>
    ...
    <PidToolbar v-if="activeTab === 'overview' && store.pidEditMode">
      <!-- Keep the theme + user controls available in normal P&ID mode. Focus
           mode omits this slot, so it stays minimal. -->
      <template #chrome>
        <HeaderUserControls @login="showLoginDialog = true" />
      </template>
    </PidToolbar>
