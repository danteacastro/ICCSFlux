...
| **Authentication** | X.509 certificates (preferred) or SAS tokens with ≤1h expiry | NIST 800-171 §3.5.3 — authenticator management |
...
| **Encryption at Rest** | AES-256, customer-managed key (CMK) via Key Vault | FIPS 140-2, NIST 800-171 §3.13.16 |
| **Encryption in Transit** | TLS 1.2+ enforced (`ssl_min_protocol_version = TLSv1.2`) | FIPS 140-2, NIST 800-171 §3.13.8 |
| **Network** | Private Endpoint only (no public access) | NIST 800-171 §3.13.1, Zero Trust |
...
| **Authentication** | Microsoft Entra ID (SSO + MFA) | NIST 800-171 §3.5.3, §3.7 |
...
| NIST 800-171 Family | Key Controls | How This Architecture Meets Them |
|---------------------|-------------|----------------------------------|
